...
import VimeoPreloader from '../components/VimeoPreloader';
import VimeoPlayer from '../components/VimeoPlayer';
import { getVideoConfig } from '../utils/videoRouter';
...
export default function Home() {
  ...
  const [currentVideo, setCurrentVideo] = useState<"introduction" | "POV_1" | "POV_2" | "POV_3" | 
    "velo" | "objet_boxe" | "objet_foot" |
    "objet_mapmonde" | "objet_sablier" | "objet_plante" | "objet_cd" |
    "objet_chien" | "objet_jeuxvideo" | "objet_photo" | "lit_vers_1" | "lit_vers_2" | "lit_vers_3">("introduction");
  ...
  // Fonction pour obtenir l'ID Vimeo d'une vidéo
  const getVimeoVideoId = (videoId: string) => {
    const config = getVideoConfig(videoId);
    return config ? config.vimeoId : '';
  };
  ...
  const handleTransition = (direction: "left" | "right") => {
    ...
    if (currentVideo === "POV_1") {
      transitionVideo = direction === "left" ? "lit_vers_2" : "lit_vers_3";
      nextVideo = direction === "left" ? "POV_2" : "POV_3";
    } else if (currentVideo === "POV_2") {
      transitionVideo = direction === "left" ? "lit_vers_1" : "lit_vers_3";
      nextVideo = direction === "left" ? "POV_1" : "POV_3";
    } else if (currentVideo === "POV_3") {
      transitionVideo = direction === "left" ? "lit_vers_1" : "lit_vers_2";
    ...
    setCurrentVideo(transitionVideo as any);
    setVideoEnded(false);
  ...
  const handleZoneClick = (zoneId: string) => {
    ...
    // La vidéo sera changée automatiquement via setCurrentVideo
    ...
    if (audioRef.current) {
      ...
      if (objectsWithMusic.includes(objetType)) {
        ...
        setTimeout(() => {
          if (audioRef.current) {
            audioRef.current.src = `https://res.cloudinary.com/dpqjlqwcq/video/upload/q_auto,f_auto/${objetType}_song`;
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      {/* Préchargeur de vidéos Vimeo */}
      <VimeoPreloader currentVideo={currentVideo} videoType={videoType} />
      
      {/* Vidéo en arrière-plan absolu */}
      <div className="absolute inset-0 w-full h-full overflow-hidden" style={{ zIndex: 0 }}>
        <VimeoPlayer
          videoId={getVimeoVideoId(currentVideo)}
